/**
 * dsh-companion — browser half smoke test.
 *
 * Executes `lib/client.js` under a stubbed DSH client environment and asserts:
 *   1. registers exactly one `__ModuleLoader__` entry with id "dsh-companion"
 *   2. the factory exports `apply` (function) and `inject` (array)
 *   3. `apply(ctx)` registers without throwing:
 *      shell.overlay pill seat, sidebar.footer.action mobile + card seats,
 *      plus the 6 settings.general.item rows
 *   4. bundle requires only react-family externals
 *
...
if (entry?.id !== 'dsh-companion') fail(`expected id "dsh-companion", got ${JSON.stringify(entry?.id)}`)
else pass('loader id is "dsh-companion"')

// bundle 只允许 require react 系（DSH 模块表运行时提供）。
{
  const code = readFileSync(CLIENT, 'utf8')
  const names = new Set()
  for (const m of code.matchAll(/require\(\s*['"]([^'"]+)['"]\s*\)/g)) names.add(m[1])
  const allow = new Set(['react', 'react-dom', 'react/jsx-runtime', 'react-dom/client'])
  const bad = [...names].filter((n) => !allow.has(n))
  if (bad.length > 0) fail(`client bundle requires unexpected modules: ${bad.join(', ')}`)
  else pass(`client bundle requires only react-family (${names.size} found)`)
}
...
for (const expected of [
  ...
  'sidebar.footer.action#dsh-mobile-plus',
  'sidebar.footer.action#dsh-done-pill-card',
  'settings.general.item#dsh-done-pill-mode',
